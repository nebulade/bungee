...
},set:function(val,silent){if(valueStore===val)return;valueStore=val;if(!silent){that.emit(name);that.emit("changed")}Quick.Engine.dirty(that,name)}})}};Quick.Element.prototype.initializeBindings=function(){var name,i;this._initializeBindingsStep=true;for(name in this._properties){if(this._properties.hasOwnProperty(name)){var value=this._properties[name];if(typeof value==="function"){var ret=this.addBinding(name,value);if(ret.hasBindings){this[name]=value}else{this[name]=ret.value}}else{this[name]=value}}}for(i in this._children){if(this._children.hasOwnProperty(i)){this._children[i].initializeBindings()}}this._initializeBindingsStep=false;this.emit("load")};Quick.Element.prototype.emit=function(signal){if(signal in this._connections){var slots=this._connections[signal];for(var slot in slots){if(slots.hasOwnProperty(slot)){slots[slot].apply()}}}};